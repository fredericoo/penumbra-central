 ...
 */
{
    ...
    class Nearby {
        ...
        init() {
            ...
            this.screenmoveFn = (ev) => requestAnimationFrame(() => {
                const docScrolls = {left : document.body.scrollLeft + document.documentElement.scrollLeft, top : document.body.scrollTop + document.documentElement.scrollTop};
                const mousepos = {x : window.innerWidth/2+docScrolls.left, y: window.innerHeight/2+docScrolls.top};
                const elRect = this.DOM.el.getBoundingClientRect();
                const elCoords = {
                    x1: elRect.left + docScrolls.left, x2: elRect.width + elRect.left + docScrolls.left,
                    y1: elRect.top + docScrolls.top, y2: elRect.height + elRect.top + docScrolls.top
            ...
            if (isMobile) {
              window.addEventListener('scroll', this.screenmoveFn);
            } else {
