...
?>
<header>
	<h3><?php esc_html_e( 'Pedidos desta assinatura', 'woocommerce-subscriptions' ); ?></h3>
</header>
				...
				<td class="order-actions woocommerce-orders-table__cell woocommerce-orders-table__cell-order-actions">
					...
					if ( $actions ) {
						foreach ( $actions as $key => $action ) {
							echo wp_kses_post( '<a href="' . esc_url( $action['url'] ) . '" class="btn btn-outline-secondary btn-block ' . sanitize_html_class( $key ) . '">' . esc_html( $action['name'] ) . '</a>' );
						}
					}
					?>
				</td>
