...
<table class="shop_table subscription_details">
	<tbody>
		...
		$dates_to_display = apply_filters( 'wcs_subscription_details_table_dates_to_display', array(
			...
			'last_order_date_created' => _x( 'Data do último pedido', 'customer subscription table header', 'woocommerce-subscriptions' ),
			'next_payment'            => _x( 'Data do próximo pagamento', 'customer subscription table header', 'woocommerce-subscriptions' ),
			'end'                     => _x( 'Data final', 'customer subscription table header', 'woocommerce-subscriptions' ),
			'trial_end'               => _x( 'Fim da degustação', 'customer subscription table header', 'woocommerce-subscriptions' ),
		), $subscription );
		...
		<?php if ( WCS_My_Account_Auto_Renew_Toggle::can_user_toggle_auto_renewal( $subscription ) ) : ?>
			<tr>
				<td><?php esc_html_e( 'Renovar automaticamente', 'woocommerce-subscriptions' ); ?></td>
				<td>
					<div class="wcs-auto-renew-toggle">
						...
						if ( $subscription->is_manual() ) {
							$toggle_label     = __( 'Ligar renovação automática', 'woocommerce-subscriptions' );
							...
							$toggle_label     = __( 'Cancelar renovação automática', 'woocommerce-subscriptions' );
			...
			<tr>
				...
				<td>
					<?php foreach ( $actions as $key => $action ) : ?>
						<a href="<?php echo esc_url( $action['url'] ); ?>" class="btn btn-outline-secondary <?php echo sanitize_html_class( $key ) ?>"><?php echo esc_html( $action['name'] ); ?></a>
					<?php endforeach; ?>
				</td>
			</tr>
		<?php endif; ?>
...
<?php if ( $notes = $subscription->get_customer_order_notes() ) : ?>
	<h3><?php esc_html_e( 'Atualizações de assinatura', 'woocommerce-subscriptions' ); ?></h3>
