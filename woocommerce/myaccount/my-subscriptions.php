...
<div class="woocommerce_account_subscriptions">
	...
	<thead>
		<tr>
			<th class="subscription-id order-number woocommerce-orders-table__header woocommerce-orders-table__header-order-number woocommerce-orders-table__header-subscription-id"><span class="nobr"><?php esc_html_e( 'Assinatura', 'woocommerce-subscriptions' ); ?></span></th>
	...
	<?php endforeach; ?>
		...
		<?php if ( 1 < $max_num_pages ) : ?>
			...
			<?php if ( 1 !== $current_page ) : ?>
				<a class="btn btn-outline-primary" href="<?php echo esc_url( wc_get_endpoint_url( 'subscriptions', $current_page - 1 ) ); ?>"><?php esc_html_e( 'Previous', 'woocommerce-subscriptions' ); ?></a>
			<?php endif; ?>

			<?php if ( intval( $max_num_pages ) !== $current_page ) : ?>
				<a class="btn btn-outline-primary" href="<?php echo esc_url( wc_get_endpoint_url( 'subscriptions', $current_page + 1 ) ); ?>"><?php esc_html_e( 'Next', 'woocommerce-subscriptions' ); ?></a>
			<?php endif; ?>
			</div>
		<?php endif; ?>
	<?php else : ?>
		<p class="no_subscriptions woocommerce-message woocommerce-message--info woocommerce-Message woocommerce-Message--info woocommerce-info">
			<?php if ( 1 < $current_page ) :
				...
				esc_html_e( 'Você não assina nenhum plano, ainda:', 'woocommerce-subscriptions' );
				?>
				<a class="btn btn-secondary" href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>">
					<?php esc_html_e( 'Mostre-me o caminho', 'woocommerce-subscriptions' ); ?>
				</a>
			<?php
		endif; ?>
		</p>
